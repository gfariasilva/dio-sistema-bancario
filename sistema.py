...
def log_transacao(func):
    def envelope(*args, **kwargs):
        ...
        with open(LOG_PATH / "log.txt", "a", encoding="utf-8") as arquivo:
            arquivo.write(
                f"[{data}] Função '{func.__name__}' executada com argumentos {args} e {kwargs}. Retornou {resultado}\n"
            )
        return resultado
...
while True:
    ...
    if opcao == "d":
        depositar(
            clientes,
            input("Informe o CPF do usuário: "),
            float(input("Digite o valor a ser depositado: ")),
        ...
        sacar(
            clientes,
            input("Informe o CPF do usuário: "),
            float(input("Digite o valor a ser sacado: ")),
            int(input("Digite o número da conta ")),
        )
    elif opcao == "e":
        print_extrato(
